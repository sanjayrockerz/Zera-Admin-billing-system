          ...
          >
            ...
            <div className="flex items-start gap-3 px-4 pt-2 pb-3 border-b border-gray-100 shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-[14px] font-black text-[#2C392A] leading-tight line-clamp-1">
                  {product.name}
                </h3>
                {selected && (
                  <div className="mt-0.5 flex items-baseline gap-1.5">
                    <span className="text-[15px] font-black text-[#2C392A] tabular-nums">
                      {formatCurrency(selected.price)}
                    </span>
                    {selected.sizeLabel && (
                      <span className="text-[11px] text-[#5F6D59]">{selected.sizeLabel}</span>
                    )}
                  </div>
                )}
            ...
            <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-3">
              <p className="mb-2.5 text-[10px] font-black uppercase tracking-wider text-[#5F6D59]">
                    ...
                    <button
                      ...
                      className={[
                        ...
                        isSelected
                          ...
                          : 'ring-1 ring-gray-200 hover:ring-[#7DAA8F] active:bg-gray-50',
                      ].join(' ')}
                    >
                      {/* Radio circle */}
                      <span
                      ...
                      <div className="flex-1 min-w-0">
                        <p
                          className={`text-[13px] font-semibold leading-tight ${
                            isSelected ? 'text-[#2C392A]' : 'text-[#333]'
                          }`}
                        >
                          {v.variantName}
                        </p>
                        {v.sizeLabel && v.sizeLabel !== v.variantName && (
                          <p className="text-[11px] text-[#5F6D59] mt-0.5">{v.sizeLabel}</p>
                        )}
                        {outOfStock && (
                      ...
                      <span
                        className={`text-[14px] font-black shrink-0 tabular-nums ${
                          isSelected ? 'text-[#2C392A]' : 'text-[#444]'
                        }`}
                      >
            ...
            >
              <div className="flex items-center gap-3">
                {/* Qty stepper */}
                <div className="flex items-center overflow-hidden rounded-xl border border-[#D5DAD0] bg-[#F7F6F2]">
                  <button
                    type="button"
                    onClick={() => setQty((q) => Math.max(1, q - 1))}
                    disabled={qty <= 1}
                    className="flex h-11 w-11 items-center justify-center text-[#2C392A] transition-colors hover:bg-[#E8EDE4] disabled:opacity-40"
                    aria-label="Decrease"
                  >
                    <Minus size={14} />
                  </button>
                  <span className="w-10 text-center text-[15px] font-black text-[#2C392A] tabular-nums">
                    {qty}
                  </span>
                  <button
                    type="button"
